...
% Test
fname_folder='E:\Ruth\h5oina\1TR\';
fname_slice_data='Bain_1TR';
phase_plot1='iron bcc (old)';
phase_plot2='iron fcc';
...
figure; 
plot(ebsd_original(phase_plot1),ebsd_original(phase_plot1).orientations); 
hold on
plot(ebsd_original(phase_plot2),ebsd_original(phase_plot2).orientations); 
nextAxis
plot(ebsd_patternmatched(phase_plot1),ebsd_patternmatched(phase_plot1).orientations);
hold on
plot(ebsd_patternmatched(phase_plot2),ebsd_patternmatched(phase_plot2).orientations);
...
oM_H1=ipfHSVKey(ebsd_patternmatched(phase_plot1).CS);
oM_H2=ipfHSVKey(ebsd_patternmatched(phase_plot2).CS);
...
plot(ebsd_patternmatched(phase_plot1),ebsd_patternmatched(phase_plot1).prop.Band_Contrast,'micronbar','on');
hold on
plot(ebsd_patternmatched(phase_plot2),ebsd_patternmatched(phase_plot2).prop.Band_Contrast,'micronbar','on');
colormap("gray")
title('BC');
nextAxis;

oM_H1.inversePoleFigureDirection = Dirx; 
oM_H2.inversePoleFigureDirection = Dirx; 
plot(ebsd_patternmatched(phase_plot1),oM_H1.orientation2color(ebsd_patternmatched(phase_plot1).orientations),'micronbar','on');
hold on
plot(ebsd_patternmatched(phase_plot2),oM_H2.orientation2color(ebsd_patternmatched(phase_plot2).orientations),'micronbar','on');
title('X');
nextAxis;

oM_H1.inversePoleFigureDirection = Diry; 
oM_H2.inversePoleFigureDirection = Diry; 
plot(ebsd_patternmatched(phase_plot1),oM_H1.orientation2color(ebsd_patternmatched(phase_plot1).orientations),'micronbar','on');
hold on
plot(ebsd_patternmatched(phase_plot2),oM_H2.orientation2color(ebsd_patternmatched(phase_plot2).orientations),'micronbar','on');
title('Y');
...
oM_H1.inversePoleFigureDirection = Dirz; 
oM_H2.inversePoleFigureDirection = Dirz; 
plot(ebsd_patternmatched(phase_plot1),oM_H1.orientation2color(ebsd_patternmatched(phase_plot1).orientations),'micronbar','on');
hold on
plot(ebsd_patternmatched(phase_plot2),oM_H2.orientation2color(ebsd_patternmatched(phase_plot2).orientations),'micronbar','on');
title('Z');
